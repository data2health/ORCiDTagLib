...
public class OtherNameDeleter extends ORCiDTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String name = null;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from orcid_dump.other_name where 1=1"
                                                        + (ID == 0 ? "" : " and id = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
                                                        );
            ...
            e.printStackTrace();
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating OtherName deleter");
        } finally {
            freeConnection();
        }
