...
public class FundingContributorDeleter extends ORCiDTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    int seqnum2 = 0;
    String orcidId = null;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from orcid_dump.funding_contributor where 1=1"
                                                        + (ID == 0 ? "" : " and id = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
            ...
            e.printStackTrace();
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating FundingContributor deleter");
        } finally {
            freeConnection();
        }
