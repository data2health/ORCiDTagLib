...
public class AffiliationCountry extends ORCiDTagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theAffiliation.commitNeeded) {
				pageContext.getOut().print(theAffiliation.getCountry());
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new JspTagException("Error: Can't find enclosing Affiliation for country tag ");
		}
		return SKIP_BODY;
	}
	...
	public String getCountry() throws JspTagException {
		try {
			Affiliation theAffiliation = (Affiliation)findAncestorWithClass(this, Affiliation.class);
			return theAffiliation.getCountry();
		} catch (Exception e) {
			e.printStackTrace();
			throw new JspTagException("Error: Can't find enclosing Affiliation for country tag ");
		}
	...
	public void setCountry(String country) throws JspTagException {
		try {
			Affiliation theAffiliation = (Affiliation)findAncestorWithClass(this, Affiliation.class);
			theAffiliation.setCountry(country);
		} catch (Exception e) {
			e.printStackTrace();
			throw new JspTagException("Error: Can't find enclosing Affiliation for country tag ");
		}
