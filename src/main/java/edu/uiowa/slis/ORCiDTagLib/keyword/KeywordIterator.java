...
public class KeywordIterator extends ORCiDTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String keyword = null;
	...
	public static String keywordCountByBio(String ID) throws JspTagException {
		int count = 0;
		KeywordIterator theIterator = new KeywordIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from orcid_dump.keyword where 1=1"
						+ " and id = ?"
						);
	...
	public static Boolean bioHasKeyword(String ID) throws JspTagException {
		return ! keywordCountByBio(ID).equals("0");
	}

	public static Boolean keywordExists (String ID, String seqnum) throws JspTagException {
		int count = 0;
		KeywordIterator theIterator = new KeywordIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from orcid_dump.keyword where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT orcid_dump.keyword.id, orcid_dump.keyword.seqnum from " + generateFromClause() + " where 1=1"
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("orcid_dump.keyword");
