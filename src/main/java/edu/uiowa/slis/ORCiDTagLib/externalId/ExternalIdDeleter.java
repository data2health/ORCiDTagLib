...
public class ExternalIdDeleter extends ORCiDTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from orcid_dump.external_id where 1=1"
                                                        + (ID == 0 ? "" : " and id = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
                                                        );
            ...
            e.printStackTrace();
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating ExternalId deleter");
        } finally {
            freeConnection();
        }
