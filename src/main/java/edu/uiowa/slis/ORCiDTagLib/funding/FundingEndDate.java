...
public class FundingEndDate extends ORCiDTagLibTagSupport {
	String type = "DATE";
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theFunding.commitNeeded) {
				...
				if (theFunding.getEndDate() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(theFunding.getEndDate());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(theFunding.getEndDate());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(theFunding.getEndDate());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(theFunding.getEndDate());
					}
				}
