...
public class EmailDeleter extends ORCiDTagLibBodyTagSupport {
    int ID = 0;
    int seqnum = 0;
    String email = null;
    boolean prime = false;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from orcid_dump.email where 1=1"
                                                        + (ID == 0 ? "" : " and id = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
                                                        );
            ...
            e.printStackTrace();
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating Email deleter");
        } finally {
            freeConnection();
        }
